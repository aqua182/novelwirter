...
def resolve_model_config(db: Session, user_id: str, novel: Novel, task_type: str, requested_id: str | None):
    group = "writing" if task_type == "generate_chapter" else "outline" if task_type in {"generate_outline","improve_outline","improve_outline_master","improve_outline_batch","plan_chapters","derive_story_plan","suggest_outline","improve_chapter_outline"} else "review"
    ...
    if task_type in {"derive_story_plan", "improve_outline_master", "improve_outline_batch"}: allowed_tasks.add("improve_outline")
